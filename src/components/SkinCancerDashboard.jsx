...
// Skin conditions that are clinically very concerning
const HIGH_RISK_CONDITIONS = ['Melanoma', 'Basal Cell Carcinoma', 'Actinic Keratosis', 'Malignant'];
...
const SkinCancerDashboard = () => {
  ...
  const isHighRisk = result ? HIGH_RISK_CONDITIONS.some(c => result.prediction.includes(c)) : false;
              ...
              {result && (
                <div style={{ animation: 'fadeIn 0.5s ease' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem', background: isHighRisk ? 'rgba(239,68,68,0.1)' : 'rgba(16,185,129,0.1)', border: `1px solid ${isHighRisk ? 'rgba(239,68,68,0.2)' : 'rgba(16,185,129,0.2)'}`, borderRadius: '12px', marginBottom: '1.5rem' }}>
                    ...
                    <div>
                      <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: '0.25rem' }}>Skin Classification</p>
                  ...
                  {result.all_probabilities && (
                    <div style={{ background: 'rgba(0,0,0,0.15)', padding: '1rem', borderRadius: '8px', marginTop: '1rem' }}>
                      ...
                      {Object.entries(result.all_probabilities).sort(([, a], [, b]) => b - a).map(([label, prob]) => (
                        <div key={label} style={{ marginBottom: '0.5rem' }}>
                          ...
                          <div style={{ width: '100%', height: '4px', background: 'rgba(255,255,255,0.08)', borderRadius: '2px', overflow: 'hidden' }}>
                            <div style={{ height: '100%', width: `${prob}%`, background: label === result.prediction ? (isHighRisk ? 'var(--risk-critical)' : 'var(--accent-teal)') : 'rgba(255,255,255,0.15)' }} />
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div style={{ marginTop: '1.25rem', padding: '1rem', borderLeft: `3px solid ${isHighRisk ? 'var(--risk-critical)' : 'var(--accent-teal)'}`, background: isHighRisk ? 'rgba(239,68,68,0.05)' : 'rgba(20,184,166,0.05)' }}>
                    ...
                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', lineHeight: '1.5' }}>
                      {isHighRisk
                        ? `${result.prediction} is a high-risk skin condition. Immediate referral to a dermatologist for biopsy and further evaluation is recommended.`
                        : `${result.prediction} is typically a lower-risk finding. A follow-up appointment with a dermatologist is advised for clinical confirmation.`}
                    </p>
                  </div>
                </div>
              )}
          ...
          {result?.heatmap && (
            <div className="glass-panel" style={{ padding: '2rem' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.8rem', marginTop: '0.75rem' }}>
                Brighter regions indicate areas the model weighted most heavily in its classification decision.
              </p>
            </div>
          )}
        </div>
